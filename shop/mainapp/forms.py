...
class RegistrationForm(forms.ModelForm):
    """
    Класс форм регистрации пользователя
    """
    ...
    def clean_email(self):
        ...
        if domain in ['com', 'net']:
            raise forms.ValidationError(f'Регистрация для "{domain}" невозможна')
        if User.objects.filter(email=email).exists():
            raise forms.ValidationError(f'Данный адрес "{email}" уже зарегистрирован в системе')
        return email
    ...
    def clean(self):
        ...
        if not last_name:
            self.cleaned_data['last_name'] = self.cleaned_data['username']
        return self.cleaned_data
