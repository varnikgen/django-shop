...
class ProductFeatures(models.Model):
    ...
    postfix_for_value = models.CharField(max_length=20, null=True, blank=True, verbose_name='Постфикс для значения',
        help_text=f'Например для характеристики "Часы работы" к значению можно добавить постфикс "часов" и получить - "10 часов"')
    use_in_filter = models.BooleanField(default=False, verbose_name='Использовать в фильтрации товаров')
    filter_type = models.CharField(max_length=20, verbose_name='Тип фильтра', default=CHECKBOX, choices=FILTER_TYPE_CHOICES)
    filter_measure = models.CharField(max_length=50, verbose_name='Единица измерения для фильтра',
        help_text='Единица измерения для конкретного фильтра. Единица измерения будет отображена в скобках.')
...
class ProductFeatureValidators(models.Model):
    """
    Проверка правильности характеристик
    """
    category = models.ForeignKey(Category, verbose_name='Категория', on_delete=models.CASCADE)
